"""Native vector storage delegated to the self-contained epistemic-graph engine."""
...
class EpistemicGraphVectorDB(VectorDB):
    """O(1)-cached clients over native indexed graph/vector operations."""
        ...
        **_kwargs: Any,
        ...
        for query in queries:
            ...
            if not keywords:
                output.append([])
                continue
            hits = client.graph.discover(keywords, [], n_results)
            values: list[tuple[Document, float]] = []
            for hit in hits:
                properties = dict(hit)
                identifier = str(properties.pop("id"))
                values.append(
                    (
                        self._document(identifier, properties),
