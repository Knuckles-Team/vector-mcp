        ...
        try:
            ...
            for _retry in range(15):
                try:
                    ...
                    print(f"[{name}] Connection attempt {_retry+1} failed ({err}). Retrying in 5s...")
            ...
            if resp.status_code != 200:
                print(f"[{name}] \033[91mFAILED\033[0m: Initial request returned {resp.status_code}")
                return False, 0
                        ...
                        if state not in ["submitted", "running", "working"]:
                            ...
                            if state == "completed" or state == "done":
                                ...
                                if "error" in poll_data["result"]:
                                     print(f"[{name}] Error details: {poll_data['result']['error']}")
            ...
            return False, time.time() - start_time
             ...
             print(f"[{name}] \033[91mFAILED\033[0m: Connection refused (is the container running?)")
             return False, 0
