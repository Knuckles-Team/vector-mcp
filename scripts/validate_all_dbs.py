...
def test_server(server_config):
    ...
    try:
        ...
        response = requests.post(f"http://localhost:{port}/mcp", json=payload, headers=headers, timeout=10)
        
        if response.status_code == 200:
            print(f"  [PASS] Connection established. Status: 200")
            ...
            resp_create = requests.post(f"http://localhost:{port}/mcp", json=create_payload, headers=headers, timeout=30)
            if resp_create.status_code == 200 and "error" not in resp_create.json():
                print(f"  [PASS] create_collection success.")
            else:
             ...
             print(f"  [WARN] 404 at /messages. Trying /sse...")
             # Just checking if we can connect to /sse
             sse_resp = requests.get(f"http://localhost:{port}/sse", stream=True, timeout=5)
             if sse_resp.status_code == 200:
                 print("  [PASS] SSE endpoint exists.")
             else:
                 print(f"  [FAIL] Could not connect to MCP server.")
        else:
